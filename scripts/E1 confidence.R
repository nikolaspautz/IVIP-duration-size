...
# Contrasts
summary(de1$condition)
(nrow <- length(levels(de1$condition)))
...
rown <- levels(de1$condition)
rownames(cmat) <- rown
  
ivs <- unique(gsub("correct_|incorrect_|", "", levels(de1$condition)))
...
# Assign contrasts
contrasts(de1$condition) <- inv.cmat
...
# Get priors
get_prior(m, data = de1) %>% tibble() %>% unique() %>% as.data.frame()
...
newdata <- make_conditions(de1, "condition")
